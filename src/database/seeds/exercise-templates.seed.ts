...
export async function seedExerciseTemplates(dataSource: DataSource) {
  ...
  // ==========================================
  // Bài 1: BT Cơ bản (BASIC)
  // ==========================================
  const exercise1 = exerciseRepo.create({
    type: ExerciseType.BASIC,
    topic: 'phep-nhan-so-thap-phan',
    scenario:
      'Chọn đúng đáp án cho các phép tính nhân số thập phân. AI sẽ nhận diện: HS sử dụng quy tắc nhân số thập phân ở mức nào? Có thể có lỗi sai: Không nhân đúng, đặt sai dấu phẩy, nhầm bản chất phép tính,...',
    timeLimit: 90, // 1 phút 30 giây
    bonusTime: 30, // nhanh trước 30 giây + bonus
    isTemplate: true,
  });
  ...
  await optionRepo.save([
    optionRepo.create({
      ...
      errorDescription:
        'Đặt sai dấu phẩy. Kết quả đúng là 4,08 (có 2 chữ số thập phân)',
    }),
    optionRepo.create({
      questionId: savedQ1_1.id,
      content: '4,3 × 3 = 12,9',
  ...
  const exercise2 = exerciseRepo.create({
    ...
    bonusTime: 60, // nhanh trước 1 phút + bonus
    isTemplate: true,
  });
  ...
  await optionRepo.save([
    optionRepo.create({
      ...
      errorDescription:
        'Hiểu sai yêu cầu bài toán, nhầm bản chất phép tính cần dùng',
    }),
    optionRepo.create({
      questionId: savedQ2_1.id,
      content: '0,75 × 19',
  ...
  // ==========================================
  // Bài 3: BT Giải quyết vấn đề (PROBLEM_SOLVING)
  // ==========================================
  const exercise3 = exerciseRepo.create({
    ...
    timeLimit: 210, // 3 phút 30 giây
    bonusTime: 90, // nhanh trước 1 phút 30 giây + bonus
    isTemplate: true,
  });
  ...
  await optionRepo.save([
    ...
    optionRepo.create({
      ...
      errorType: 'misunderstanding',
      errorDescription:
        'HS chưa nắm được dữ kiện đề bài: Cửa hàng khuyến mãi mua 15 tặng 1',
    }),
    optionRepo.create({
      questionId: savedQ3_4.id,
      content: '14 dây',
      ...
      errorType: 'misunderstanding',
      errorDescription:
        'HS chưa nắm được dữ kiện đề bài: Không đúng với điều kiện cửa hàng đưa ra',
    }),
    optionRepo.create({
      questionId: savedQ3_4.id,
      content: 'Mình chưa chắc.',
  ...
  await optionRepo.save([
    optionRepo.create({
      ...
      errorType: 'calculation_error',
      errorDescription:
        'Mỗi dây sau giảm còn 10,5 nghìn đồng, khi mua 16 dây thì tổng là 168 nghìn đồng. Còn ở cửa hàng B chỉ có 10,8 x 15 = 162 nghìn đồng',
    }),
    optionRepo.create({
      questionId: savedQ3_6.id,
      content: 'Cửa hàng B',
